...
const TrendingItems = ({error, isLoading, results, checked}) => {
  return (
    error ? <Error>{error}</Error> : 
    isLoading || !results ? (<Loading />) : (
    <ul className={styles.trending__list}>
      {results.results.map((result, index) => (
        <li key={result.id} className={`${styles.trending__list__item} ${index >= 8 ? styles.hide : ''}`} >
          <a href={`/${checked === 'movie' ? 'movies' : 'shows'}/${result.id}`} className={styles.trending__list__item__link} >
            <div className={styles.trending__list__item__link__img} >
              <img src={`${movieDatabase.baseUrlImage}w500${result.poster_path}`} alt={`Poster of ${result.title}`} />
            </div>
    
            <div className={styles.trending__list__item__link__text} >
